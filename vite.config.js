...
// to remove console.log: vite-plugin-remove-console
export default defineConfig({
  ...
  build: {
    sourcemap: false,
    minify: 'esbuild', // or 'terser', 'esbuild' is faster and 'terser' is more powerful for JS Minification
    target: 'es2017',  // smaller JS output
    cssMinify: true,
    chunkSizeWarningLimit: 1000, // 1000kb = 1mb, avoid warning about chunk size
    rollupOptions: {
      output: {
        entryFileNames: 'js/[name]-[hash].js', // main entry JS
        chunkFileNames: 'js/[name]-[hash].js', // dynamic JS chunks
        assetFileNames: ({ name }) => {
